...
async function runBuildLambda(inputPath) {
  ...
  const analyzeResult = runAnalyze(wrapper, { files: inputFiles, entrypoint, config: build.config })
  console.log(analyzeResult)
  const workPath = await getWritableDirectory();
  const buildResult = await wrapper.build({ files: inputFiles, entrypoint, config: build.config, workPath });
  return {
    analyzeResult,
    buildResult
  }
