...
module.exports = async function(files, opts = {}) {
  ...
  if (opts.isDir) job.isDir = opts.isDir;
...
class Job {
  constructor({ base, filterBase, ignore, log = false }) {
    if (base) {
  ...
  emitFile(path, reason, parent) {
    ...
    if (this.base) {
      ...
      if (parent) parent = relative(this.base, parent);
    }
    const reasonEntry =
      this.reasons[path] ||
      (this.reasons[path] = {
  ...
  async emitDependency(path, parent) {
    ...
    await Promise.all([
      ...[...assets].map(async asset => {
        ...
        else this.emitFile(asset, 'asset', path);
      }),
      ...[...deps].map(async dep => {
        try {
